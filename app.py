...
@app.route('/galeria/comprados')
def tutorial():
    return render_template('comprados.html')

@app.route('/galeria/recetas')
def tutorial():
    return render_template('recetas.html')

@app.route('/galeria/nevera')
def tutorial():
    return render_template('nevera.html')
...
@app.route('/galeria/familia')
def tutorial():
    return render_template('familia.html')

@app.route('/galeria/cerrarSesion')
def tutorial():
...
def updateFamilia(df, csv_address, commit="Updating family content"):
    
    df.to_csv(index=False, path_or_buf=csv_address)
...
def add_item(edit: bool=False, adddata=None):
    ...
    logger.info(f"Adding item: ID={ID}, Location={Location}, Amount={Amount}, Parent={Parent}, Type={Type}")
    ...
    updateDataframe(df, csv_file_path, f"Added {ID} to the database")
...
def del_item(edit: bool=False, deldata=None):
    ...
    logger.info("delete item: " + ID, Location, Amount)

    df = pd.read_csv(csv_file_path)

    logger.debug(df)
    logger.debug(f"dataframe fetched from {csv_file_path} successfully " + "_"*30)
    ...
    if int(Amount) == int(matching_rows["Amount"].iloc[0]):
        ...
        updateDataframe(df, csv_file_path, f"Deleted {ID} from the database")
        if not edit:
    ...
    # If User provides ID, Location and a different Amount of element:
        ...
        updateDataframe(df, csv_file_path, f"Deleted {Amount} units of {ID} from the database")
        if not edit:
